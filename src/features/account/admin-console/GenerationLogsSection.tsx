...
export function GenerationLogsSection(props: GenerationLogsSectionProps) {
  const {
    ...
    selectedIds, batchOperating, setQuery, setStatus, setModelFilter, setFrom, setTo,
    setPage, onToggleTask, onTogglePage, onClearSelection, onBatchDelete, onOpenDetail, onDelete,
  } = props
      ...
      <AdminTableShell mobileHint="横向滑动查看更多任务字段和操作" footer={<PaginationBar page={page} pageSize={pageSize} total={total} onPageChange={setPage} />}>
        <div className="min-w-[1500px]">
          <div className="sticky top-0 z-20 grid grid-cols-[34px_1.45fr_170px_1.05fr_1fr_180px_110px_90px_150px_170px] gap-3 border-b border-gray-100 bg-gray-50 px-4 py-3 text-xs font-semibold text-gray-500 dark:border-white/[0.06] dark:bg-[#171a22]">
            ...
            <span>请求 / 输出</span><span>类型 / 参考图</span><span>用户</span><span>模型</span><span>参数</span><span>状态</span><span>消耗</span><span>创建时间</span><span className="text-right">操作</span>
          </div>
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className={cx('grid grid-cols-[34px_1.45fr_170px_1.05fr_1fr_180px_110px_90px_150px_170px] items-center gap-3 border-b border-gray-100 px-4 py-3 text-sm transition last:border-0 dark:border-white/[0.06]', checked ? 'bg-blue-50/70 dark:bg-blue-400/10' : 'hover:bg-gray-50 dark:hover:bg-white/[0.04]')}>
                ...
                <StatusBadge tone={taskTone(task.status)}>{task.status}</StatusBadge><div className="font-semibold text-amber-700">{task.costCredits}</div><span className="text-xs text-gray-400">{formatTime(task.createdAt)}</span>
